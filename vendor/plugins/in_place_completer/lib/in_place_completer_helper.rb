module InPlaceCompleterHelper
  ...
  def in_place_editor_local_completer(field_id, completions, options = { }, completion_options = { })
    ...
    js_options['blankText'] = %('#{options[:blank_text]}') if options[:blank_text]
    ...
    js_options['onFormCustomization'] = "function(ipe, form) { #{options[:customize_form]} }" if options[:customize_form]
    js_options['submitOnBlur'] = "true" if options[:submit_on_blur]
    js_options['onEnterEditMode'] = "function(ipe) { #{options[:enter_edit_mode]} }" if options[:enter_edit_mode]
    js_options['onSaving']   = "function(ipe) { #{options[:saving]} }" if options[:saving]
    js_options['onComplete']   = "function(transport, element) { #{options[:complete]} }" if options[:complete]
    js_options['onSuccess']   = "function(ipe) { #{options[:success]} }" if options[:success]
    js_options['onFailure']   = "function(ipe, transport) { #{options[:failure]} }" if options[:failure]
  ...
  def in_place_completing_editor_field(object, method, tag_options = {}, in_place_editor_options = {}, completion_options = {})
    ...
    in_place_editor_options[:controller] ||= controller_name
    set_url = url_for({ :controller => in_place_editor_options[:controller],
                        :action => "set_#{object}_#{method}",
                        :id => tag.object.id })
    in_place_editor_options = in_place_editor_options.merge(:url => set_url)
    completion_options[:url] ||= url_for \
      :controller => in_place_editor_options[:controller],
      :action => "auto_complete_for_#{object}_#{method}"
    completion_options[:update] ||=  "#{object}_#{method}_#{tag.object.id}_auto_complete"
    value =  tag.value(tag.object)
    format = tag_options.delete(:format)
    value = format % value unless format.blank?
    (completion_options[:skip_style] ? "" : auto_complete_stylesheet) +
      tag.content_tag(tag_options.delete(:tag), h(value), tag_options) +
      content_tag("div", "", :id => completion_options[:update], :class => "auto_complete", :style => "position: absolute; left: 0px; top: 0px;") +
  ...
  def in_place_local_completing_editor_field(object, method, completions, tag_options = {}, in_place_editor_options = {}, completion_options = {})
    tag = ::ActionView::Helpers::InstanceTag.new(object, method, self)
    tag_options = {:tag => "span", :id => "#{object}_#{method}_#{tag.object.id}_in_place_editor", :class => "in_place_editor_field"}.merge!(tag_options)
    in_place_editor_options = in_place_editor_options.merge(:url => url_for({ :action => "set_#{object}_#{method}", :id => tag.object.id }))
    completion_options[:update] ||=  "#{object}_#{method}_#{tag.object.id}_auto_complete"
    value =  tag.value(tag.object)
    format = tag_options.delete(:format)
    value = format % value unless format.blank?
