...
module CalendarDateSelect::FormHelpers
  ...
  def calendar_date_select_tag( name, value = nil, options = {})
    image, image_options, options, javascript_options = calendar_date_select_process_options(options)
    ...
    calendar_date_select_output(tag, image, image_options, options, javascript_options)
  end
  ...
  private
    ...
    def calendar_date_select_process_options(options)
      ...
      [image, image_options, options, javascript_options]
    end

    def calendar_date_select_output(input, image, image_options = {}, options = {}, javascript_options = {})
      out = input
      if javascript_options[:embedded]
        ...
        out << " "
        onclick_js = "new CalendarDateSelect( $(this).previous(), #{options_for_javascript(javascript_options)} );"
        default_image_options = { :onclick => onclick_js,
          :style => 'border:0px; cursor:pointer;',
          :class => 'calendar_date_select_popup_icon' }
        out << image_tag(image, default_image_options.merge!(image_options))
      end
      out
    end
